...
over the records left. Each range comes from refitting to many resamples of that decay (a
parametric bootstrap).

It is fitted twice — to the recent half of the history and to all of it — and the answer
spans both. Either alone was overconfident on 21 labelled SYNERGY reviews: the recent fit's
90% range held the true number only 72% of the time and fell short of it 20%; together
they held it 87% of the time and fell short 6% (`benchmarks/stopping.py`). The expected
number is the larger of the two. See docs/methods.md.
"""
...
def estimate_remaining(relevant: Sequence[bool], remaining: int, *, seed: int = 0) -> Estimate:
    ...
    include or a maybe.
    """
    if remaining <= 0 or not relevant:
        return Estimate(expected=0.0, low=0, high=0)
    recent = _estimate(relevant[-max(MIN_WINDOW, len(relevant) // 2) :], remaining, seed)
    whole = _estimate(relevant, remaining, seed)

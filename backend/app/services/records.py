...
from app.models import FullTextStatus, ImportBatch, Record, TitleAbstractStatus
...
class RecordService:
    ...
    def _filtered(
        ...
        *,
        ...
        if ta is not None:
            statement = statement.where(Record.ta_final == ta)
        if ft is not None:
            statement = statement.where(Record.ft_final == ft)
    ...
    async def page(
        ...
        *,
        ...
        items = [_out(record) for record in rows[:limit]]
    ...
    async def detail(self, access: ProjectAccess, record_id: uuid.UUID) -> RecordDetail:
        ...
        record, source = row
        return RecordDetail(**_out(record).model_dump(), **_extra(record), source=source)
...
def _out(record: Record) -> RecordOut:
    return RecordOut(
        ...
        doi=record.doi,
        pmid=record.pmid,
        ta_final=record.ta_final,
        ft_final=record.ft_final,
